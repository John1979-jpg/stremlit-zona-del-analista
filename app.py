...
def process_events_dataframe(df, teams_dict):
    ...
    df['cumulative_mins'] = df['minute'] + (1 * (df['period'] == 2)) * 45 + df['second'] / 60
...
def get_passes_between_df(team_name, passes_df, players_df):
    ...
    team_passes = passes_df[passes_df['teamName'] == team_name].copy()
    
    if team_passes.empty:
        return pd.DataFrame(), pd.DataFrame()
    
    average_locs = team_passes.groupby('playerId').agg({
        'x': 'mean',
        'y': 'mean'
    }).reset_index()
    average_locs.columns = ['playerId', 'pass_avg_x', 'pass_avg_y']
    
    pass_counts = team_passes.groupby('playerId').size().reset_index(name='count')
    
    average_locs_and_count_df = average_locs.merge(pass_counts, on='playerId')
    ...
    if 'receiver' not in team_passes.columns:
        return pd.DataFrame(), average_locs_and_count_df
    
    passes_player_ids_df = team_passes[['playerId', 'receiver']].dropna()
...
def plot_pass_network(ax, passes_between_df, average_locs_and_count_df, team_color, team_name, bg_color, line_color, is_away=False):
    ...
    avg_height = average_locs_and_count_df['pass_avg_x'].median()
    ax.axvline(x=avg_height, color='gray', linestyle='--', alpha=0.75, linewidth=2)
    
    if is_away:
        ax.invert_xaxis()
        ax.invert_yaxis()
        ax.text(avg_height - 1, 73, f"{avg_height:.1f}m", fontsize=12, color=line_color, ha='left')
    else:
        ax.text(avg_height - 1, -5, f"{avg_height:.1f}m", fontsize=12, color=line_color, ha='right')
    ...
    return {
        'Team': team_name,
        'Avg_Pass_Height': round(avg_height, 2),
        'Total_Passes': average_locs_and_count_df['count'].sum()
    }
...
def plot_progressive_passes(ax, df, team_name, team_color, bg_color, line_color, is_away=False):
    ...
    df_team = df[(df['teamName'] == team_name) & (df['type'] == 'Pass') & (df['outcomeType'] == 'Successful')].copy()
    
    if df_team.empty:
        ax.text(52.5, 34, "No hay datos suficientes", ha='center', va='center', fontsize=14, color=line_color)
        return {}
    
    df_team['prog_pass'] = np.sqrt((105 - df_team['x'])**2 + (34 - df_team['y'])**2) - \
                           np.sqrt((105 - df_team['endX'])**2 + (34 - df_team['endY'])**2)
    
    dfpro = df_team[(df_team['prog_pass'] >= 9.11) & (df_team['x'] >= 35)]
    ...
    ax.text(8, 11.335, f'{right_pro}\n({right_pro*100//pro_count}%)', color=team_color, fontsize=18, va='center', ha='center', bbox=bbox_props)
    ax.text(8, 34, f'{mid_pro}\n({mid_pro*100//pro_count}%)', color=team_color, fontsize=18, va='center', ha='center', bbox=bbox_props)
    ax.text(8, 56.675, f'{left_pro}\n({left_pro*100//pro_count}%)', color=team_color, fontsize=18, va='center', ha='center', bbox=bbox_props)
    
    pitch.lines(dfpro['x'], dfpro['y'], dfpro['endX'], dfpro['endY'], lw=3, comet=True, color=team_color, ax=ax, alpha=0.5)
...
def plot_progressive_carries(ax, df, team_name, team_color, bg_color, line_color, is_away=False):
    ...
    df_team = df[(df['teamName'] == team_name) & (df['type'] == 'Carry')].copy()
    
    if df_team.empty:
        ax.text(52.5, 34, "No hay datos suficientes", ha='center', va='center', fontsize=14, color=line_color)
        return {}
    
    df_team['prog_carry'] = np.sqrt((105 - df_team['x'])**2 + (34 - df_team['y'])**2) - \
                            np.sqrt((105 - df_team['endX'])**2 + (34 - df_team['endY'])**2)
    
    dfpro = df_team[(df_team['prog_carry'] >= 9.11) & (df_team['x'] >= 35)]
    ...
    bbox_props = dict(boxstyle="round,pad=0.3", edgecolor="None", facecolor=bg_color, alpha=0.75)
    ax.text(8, 11.335, f'{right_pro}', color=team_color, fontsize=18, va='center', ha='center', bbox=bbox_props)
    ax.text(8, 34, f'{mid_pro}', color=team_color, fontsize=18, va='center', ha='center', bbox=bbox_props)
    ax.text(8, 56.675, f'{left_pro}', color=team_color, fontsize=18, va='center', ha='center', bbox=bbox_props)
...
def plot_shotmap_combined(ax, df, home_team, away_team, home_color, away_color, bg_color, line_color):
    ...
    # Tiros del equipo local (hacia la derecha, invertimos para mostrar en la izquierda)
    ...
    # Tiros del equipo visitante (hacia la izquierda)
    ...
    # Títulos
    home_goals = len(hGoalData)
    away_goals = len(aGoalData)
    ...
    ax.set_title("Mapa de Tiros", color=line_color, fontsize=20, fontweight='bold')
    
    return {
        home_team: {'Goals': home_goals, 'Shots': len(home_shots)},
        away_team: {'Goals': away_goals, 'Shots': len(away_shots)}
    }
...
def plot_shotmap_individual(ax, df, team_name, team_color, bg_color, line_color):
    ...
    if shots_df.empty:
        ...
        return {'Team': team_name, 'Goals': 0, 'Shots': 0}
...
def main():
    ...
    st.markdown('<p class="sub-header">Análisis de partidos con datos de WhoScored</p>', unsafe_allow_html=True)
    ...
    if uploaded_file is not None:
        try:
            ...
            # Mostrar metadatos si están completos
            ...
            tabs = st.tabs([
                ...
                "📈 Pases Progresivos",
                "🏃 Conducciones",
                "⚽ Mapa de Tiros (Combinado)",
                "🎯 Mapa de Tiros (Individual)",
            ...
            with tabs[0]:
                ...
                home_passes_between, home_avg_locs = get_passes_between_df(home_team, passes_df, players_df)
                away_passes_between, away_avg_locs = get_passes_between_df(away_team, passes_df, players_df)
                
                fig, axes = plt.subplots(1, 2, figsize=(20, 10), facecolor=bg_color)
                
                home_stats = plot_pass_network(axes[0], home_passes_between, home_avg_locs, 
                                              home_color, home_team, bg_color, line_color, is_away=False)
                away_stats = plot_pass_network(axes[1], away_passes_between, away_avg_locs, 
            ...
            # Tab 5: Mapa de Tiros Individual
            with tabs[4]:
        ...
        with st.expander("🔧 Visualizaciones disponibles"):
            st.markdown("""
            - **Pass Network**: Red de pases de cada equipo
            - **Pases Progresivos**: Pases que avanzan significativamente hacia portería
            - **Conducciones**: Carreras con balón progresivas
            - **Mapa de Tiros (Combinado)**: Ambos equipos en un campo
            - **Mapa de Tiros (Individual)**: Cada equipo por separado con estadísticas
